...
export default function ProductDetailPage({ cars }: ProductPageProps) {
    ...
    const handleQuantityChange = (action: "increase" | "decrease") => {
        if (action === "increase" && cars && quantity < cars.quantity) {
    ...
    const handleAddToCart = () => {
        if (!cars) return;
        toast.success(`Added ${cars.name} to cart!`);
    };
    ...
    const discountedPrice =
        cars?.discount && cars.price
            ? cars.price * (1 - cars.discount / 100)
            : null;

    // Safe stock check
    const inStock = (cars?.quantity ?? 0) > 0 && cars.status !== "Out of Stock";
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-muted-foreground mb-4">The cars you are looking for doesn’t exist.</p>
                    <Button asChild>
    ...
    return (
        <div className="min-h-screen  mt-32">
            {/* Breadcrumb */}
            <section className="py-4 px-4 border-b border-white/20">
                <div className="container mx-auto">
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                        ...
                        <span>/</span>
                        <Link href="/market-cars" className="hover:text-foreground transition-colors">
                            Marketplace
                        </Link>
                        <span>/</span>
                        <Link
                        ...
                        <span>/</span>
                        <span className="text-foreground font-medium truncate">{cars.name}</span>
                    </div>
                </div>
            </section>
            ...
            <section className="py-8 px-4">
                <div className="container mx-auto">
                    <Button variant="ghost" size="sm" onClick={() => router.back()} className="mb-6 text-white hover:cursor-pointer">
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 mb-12">
                        ...
                        <div className="space-y-4">
                            <Card className="overflow-hidden bg-white/10">
                                <CardContent className="p-0 ">
                                    <div className="relative aspect-square">
                                        <Image
                                            src={cars.imagesOnCars[selectedImage]?.imageUrl || "/placeholder.svg"}
                                            alt={cars.name}
                                            fill
                            ...
                            {/* Thumbnail Images */}
                            {cars.imagesOnCars.length > 1 && (
                                <div className="grid grid-cols-4 gap-4">
                                    {cars.imagesOnCars.map((image, index) => (
                                        <button
                                            key={image.id}
                            ...
                            <div>
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex-1">
                                        <h1 className="text-3xl font-bold mb-2 text-white">{cars.name}</h1>
                                ...
                                <div className="flex items-center gap-2 mb-4">
                                    ...
                                    >
                                        {inStock ? `In Stock (${cars.quantity})` : "Out of Stock"}
                                    </Badge>
                                </div>
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex border-white/20 items-center border rounded-lg">
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                            ...
                                            <div className="px-6 py-2 font-semibold bg-white/10 min-w-[60px] text-center">{quantity}</div>
                                            <Button
                                                variant="ghost"
                                                size="icon"
                                                onClick={() => handleQuantityChange("increase")}
                                                disabled={quantity >= cars.quantity || !inStock}
                                                className="rounded-l-none"
                                            >
                                                <Plus className="h-4 w-4" />
                                            </Button>
                                        </div>
                                        {inStock && <span className="text-sm text-muted-foreground">{cars.quantity} available</span>}
                                    </div>
                                </div>
                        ...
                        <TabsContent value="description" className="mt-6">
                            <Card>
                                ...
                                <CardContent className="prose max-w-none">
                                    <p className="text-muted-foreground leading-relaxed">{cars.description}</p>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        <div className="space-y-3">
                                            <div className="flex justify-between py-2 border-b">
                                                ...
                                                <span className="text-muted-foreground">{cars.category.name}</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b">
                                                <span className="font-medium">Model</span>
                                                <span className="text-muted-foreground">{cars.model.name}</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b">
                                                <span className="font-medium">Status</span>
                                                <Badge variant="outline">{cars.status}</Badge>
                                            </div>
                                            <div className="flex justify-between py-2 border-b">
                                                <span className="font-medium">Product ID</span>
                                                <span className="text-muted-foreground font-mono text-xs">{cars.id}</span>
                                            </div>
                                        </div>
                                        <div className="space-y-3">
                                            <div className="flex justify-between py-2 border-b">
                                                <span className="font-medium">Availability</span>
                                                <span className="text-muted-foreground">{cars.quantity} in stock</span>
                                            </div>
                                            <div className="flex justify-between py-2 border-b">
                        ...
                        <TabsContent value="reviews" className="mt-6">
                            <Card>
                                <CardHeader>
                                    ...
                                    <CardDescription>See what our customers are saying about this cars</CardDescription>
                                </CardHeader>
                                <CardContent>
                                    <div className="space-y-6">
                                        {/* Review Summary */}
                                            ...
                                            {[1, 2, 3].map((i) => (
                                                <div key={i} className="space-y-3">
                                                    ...
                                                    <p className="text-muted-foreground">
                                                        Excellent cars! The quality exceeded my expectations. Fast shipping and great customer
